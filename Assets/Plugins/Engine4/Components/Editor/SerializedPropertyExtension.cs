...
namespace Engine4.Editing {
 ...
 //{
     ...
     public static class SerializedPropertyExtensions 
     {
         ...
         public static bool SetValueAll<T>(this SerializedProperty property, T value) where T:ICloneable
         {
             
             var objs = GetSerializedPropertyRootComponents(property);
             //Iterate to parent object of the value, necessary if it is a nested object
             string[] fieldStructure = property.propertyPath.Split('.');
            ...
            foreach (var obj in objs)
            {
                ...
                for (int i = 0; i < fieldStructure.Length - 1; i++)
                 {
                     o = GetFieldOrPropertyValue<object>(fieldStructure[i], o);
                 }
                 string fieldName = fieldStructure.Last();
     
                 SetFieldOrPropertyValue(fieldName, o, objs.Length > 1 ? value.Clone() : value); //It must be cloned to prevent unexpected cross-editing
            }
            return true;
         }

        public static bool SetStructValueAll<T>(this SerializedProperty property, T value)
         {
             ...
             string[] fieldStructure = property.propertyPath.Split('.');
                ...
                foreach (var obj in objs)
                {
                    ...
                    for (int i = 0; i < fieldStructure.Length - 1; i++)
                     ...
                     SetFieldOrPropertyValue(fieldName, o, value); //It must be cloned to prevent unexpected cross-editing
                }
                return true;
         }
        ...
        public static UnityEngine.Object[] GetSerializedPropertyRootComponents(SerializedProperty property)
         {
